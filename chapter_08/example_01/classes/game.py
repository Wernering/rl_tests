...
class Game:
    ...
    def step(self, action: int) -> None:
        ...
        if action == 0:
            ...
            logger.debug(
                f"Action: 'up'. Probe position: {get_value_as_matrix(probe_position)}"
            )
            possible = False
            ...
            probe_position = self.player.current_position + [1, 0]
            logger.debug(
                f"Action: 'down'. Probe position: {get_value_as_matrix(probe_position)}"
            )
            possible = False
            ...
            logger.debug(
                f"Action: 'left'. Probe position: {get_value_as_matrix(probe_position)}"
            )
            possible = False
            ...
            probe_position = self.player.current_position + [0, 1]
            logger.debug(
                f"Action: 'right'. Probe position: {get_value_as_matrix(probe_position)}"
            )
            possible = False
