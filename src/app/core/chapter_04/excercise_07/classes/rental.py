...
class JacksRental:
    ...
    def __init__(
        ...
        **kw,
        ...
        self.parking_cost = -abs(kw.get("parking_cost", 4))
        self.free_cars = kw.get("free_cars", 10)
        self.free_a_to_b = kw.get("free_a_to_b", 1)
    ...
    def calculate_expected_value(self, state: tuple, action: int, v_matrix: np.ndarray):
        ...
        non_free_moves = action
        if action > 0:
            non_free_moves -= self.free_a_to_b

        v += abs(non_free_moves) * self.cost
            ...
            for lease_b, prob_lease_b in self.B.lease_probability.items():
                for return_a, prob_return_a in self.A.return_probability.items():
                    for return_b, prob_return_b in self.B.return_probability.items():
                        probability = prob_lease_a * prob_lease_b * prob_return_a * prob_return_b
                        ...
                        if end_car_a > self.free_cars:
                            reward += self.parking_cost

                        if end_car_b > self.free_cars:
                            reward += self.parking_cost

                        v += probability * (reward + self.discount * v_matrix[end_car_a][end_car_b])
    ...
    def policy_improvement(self) -> bool:
        ...
        for state_a in range(self.A.capacity + 1):
            for state_b in range(self.B.capacity + 1):
                ...
                new_action = [key for key, value in v_dict.items() if value == max(v_dict.values())][0]
