...
class JacksRental:
    ...
    def expected_return(self, state, action, vf_matrix):
        ...
        # nonlineal condition: The first car from A to B is free
        if action > 0:
            vf += abs(action - 1) * self.cost
        else:
            vf += abs(action) * self.cost

        # If there are more than 10 cars in a location at the end of the day (after making the action),
        # there is a fix price
