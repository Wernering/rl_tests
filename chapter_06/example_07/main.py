...
class Agent:
    def __init__(self, update_method="Double Q-Learning", b_card=10):
        ...
        # Nodes
        self.Terminal = {"Q1": np.zeros(1), "Q2": np.zeros(1)}

        self.B = {
            "actions": {i: self.Terminal for i in range(b_card)},
            "Q1": np.zeros(b_card),
            "Q2": np.zeros(b_card),
        }

        self.A = {
            "actions": {0: self.Terminal, 1: self.B},
            "Q1": np.zeros(2),
            "Q2": np.zeros(2),
        }
    ...
    def q_learning_update(self, initial_state, action, end_state):
        initial_state["Q1"][action] = initial_state["Q1"][action] + self.alpha * (
            self.reward() + max(end_state["Q1"]) - initial_state["Q1"][action]
        )

    def double_q_learning_update(self, initial_state, action, end_state):
        if np.random.random() < 0.5:
            argmax = np.random.choice(
                np.flatnonzero(end_state["Q2"] == end_state["Q2"].max())
            )
            initial_state["Q1"][action] = initial_state["Q1"][action] + self.alpha * (
                self.reward() + end_state["Q1"][argmax] - initial_state["Q1"][action]
            )
        else:
            argmax = np.random.choice(
                np.flatnonzero(end_state["Q1"] == end_state["Q1"].max())
            )
            initial_state["Q2"][action] = initial_state["Q2"][action] + self.alpha * (
                self.reward() + end_state["Q2"][argmax] - initial_state["Q2"][action]
            )
